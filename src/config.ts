...
export function GetConfigFields(): SomeCompanionConfigField[] {
  return [
    ...
    },
    {
      ...
      default: '',
      required: false,
      tooltip: 'Optional: List of device IPs for multi-device control in a single instance. Leave empty to use the single Host/Port above.'
      ,isVisible: (config) => (config as any).mode === 'multi',
    },
    {
      type: 'number',
    ...
    },
    {
      ...
      width: 6,
      default: false,
      tooltip: 'When enabled, the module will read power/mutes/alarms via UDP port 1234. Writes continue to use HTTP API.'
    },
    {
      type: 'number',
      id: 'udpPort',
