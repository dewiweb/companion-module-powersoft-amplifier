...
const BROADCAST_ARG = ((process.argv[6] || '') + ' ' + (process.argv[7] || '') + ' ' + (process.argv[8] || '') + ' ' + (process.argv[9] || ''))
  .toLowerCase()
  .includes('broadcast')
...
const GENCRC_ARG = ((process.argv[6] || '') + ' ' + (process.argv[7] || '') + ' ' + (process.argv[8] || '') + ' ' + (process.argv[9] || ''))
  .toLowerCase()
  .includes('gencrc')
// Force CRC16=0 for any command (debugging some firmwares)
const ZEROCRC_ARG = ((process.argv[6] || '') + ' ' + (process.argv[7] || '') + ' ' + (process.argv[8] || '') + ' ' + (process.argv[9] || '') + ' ' + (process.argv[10] || ''))
  .toLowerCase()
  .includes('zerocrc')
...
function buildFrame({ cmd, cookie, answerPort, data, forceComputedCrcForCmd14 = false, forceZeroCrc = false }) {
  ...
  let o = 0
  buf.writeUInt8(STX, o); o += 1
  buf.writeUInt8(cmd, o); o += 1
  buf.writeUInt16LE(cookie, o); o += 2
  buf.writeUInt16LE(count, o); o += 2
  buf.writeUInt16LE(answerPort, o); o += 2
  if (count) data.copy(buf, o), o += count
  buf.writeUInt16LE(crc, o); o += 2
  buf.writeUInt8(notCmd, o); o += 1
  buf.writeUInt8(ETX, o); o += 1
  return buf
}
...
function parseFrame(buf) {
  try {
    let o = 0
    const STX = buf.readUInt8(o); o += 1
    const cmd = buf.readUInt8(o); o += 1
    const cookie = buf.readUInt16LE(o); o += 2
    const count = buf.readUInt16LE(o); o += 2
    const answerPort = buf.readUInt16LE(o); o += 2
    const data = buf.slice(o, o + count); o += count
    const crc16 = buf.readUInt16LE(o); o += 2
    const notCmd = buf.readUInt8(o); o += 1
    const ETX = buf.readUInt8(o); o += 1
...
function hex(buf) {
  return Buffer.isBuffer(buf) ? buf.toString('hex').match(/.{1,2}/g)?.join(' ') : String(buf)
}
...
async function main() {
  ...
  frame = buildFrame({ cmd, cookie, answerPort: answerPortField, data: payload, forceComputedCrcForCmd14: (cmd === 14 && GENCRC_ARG), forceZeroCrc: ZEROCRC_ARG })
  console.log(`Sending ${CMD_ARG.toUpperCase()} frame:`, hex(frame))
  if (cmd === 14) console.log(`  standby mode: ${payload[0]} (0=read,1=off,2=on) | broadcast=${BROADCAST_ARG} | answer_port=${answerPortField} | crc_mode=${ZEROCRC_ARG ? 'forced_zero' : (GENCRC_ARG ? 'general(computed)' : 'special(0x0000)')}`)
  if (cmd === 3) console.log(`  channel=${payload[0]} | outmute=${payload[1]} (0=unmute,1=mute) | broadcast=${BROADCAST_ARG} | answer_port=${answerPortField} | crc_mode=${ZEROCRC_ARG ? 'forced_zero' : 'computed'}`)
  ...
  sock.on('message', (msg, rinfo) => {
    ...
    if (cmd === 1 && parsed.data && parsed.data.length >= 2) {
      ...
      let off = 0
      const answer_ok = dv.readUInt8(off); off += 1
      const num_channels = dv.readUInt8(off); off += 1
      ...
      for (let i = 0; i < num_channels && off + 2 <= dv.length; i++) {
        ingain.push(dv.readInt16LE(off)); off += 2
      }
      // OUTGAINx (int16 LE) in centi-dB
      for (let i = 0; i < num_channels && off + 2 <= dv.length; i++) {
        outgain.push(dv.readInt16LE(off)); off += 2
      }
      // INMUTEx (u8)
      for (let i = 0; i < num_channels && off + 1 <= dv.length; i++) {
        inmute.push(dv.readUInt8(off)); off += 1
      }
      // OUTMUTEx (u8)
      for (let i = 0; i < num_channels && off + 1 <= dv.length; i++) {
        outmute.push(dv.readUInt8(off)); off += 1
      }
    ...
    if (cmd === 15 && parsed.data) {
      ...
      if (dv.length >= 2) {
        ...
        console.log('READALLALARMS decoded:', { answer_ok, alarms_hex: '0x' + alarms.toString(16).padStart(2, '0'), alarms_bin: bin, dataLen: dv.length })
      } else {
    ...
    if (cmd === 25 && parsed.data) {
      ...
      if (dv.length < 4) {
        ...
        const answer_ok = dv.readUInt8(off); off += 1
        const gpio_alarms = dv.readUInt8(off); off += 1
        // two reserved bytes
        off += 2
        const words = []
        while (off + 4 <= dv.length) {
          words.push(dv.readUInt32LE(off)); off += 4
        }
