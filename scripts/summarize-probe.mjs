...
console.log(`Meta: host=${doc?.meta?.host} port=${doc?.meta?.port} https=${doc?.meta?.https} channels=${doc?.meta?.channels}`)
...
for (const [key, arr] of [...byKey.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  const indices = arr
    .map((x) => (x.i0 != null && x.i1 != null ? `[${x.i0}${x.id.includes('{1}') ? ',' + x.i1 : ''}]` : ''))
  console.log(`- ${key} -> count=${arr.length}, types={${[...types].join(', ')}}, sampleId=${sample[0]}`)
}

if (csvOut) {
  const lines = [
    'key,id,i0,i1,valueType,field'
  ]
  for (const r of ok) {
    lines.push(`${JSON.stringify(r.key)},${JSON.stringify(r.id)},${r.i0 ?? ''},${r.i1 ?? ''},${r.valueType ?? ''},${JSON.stringify(r.field ?? '')}`)
  }
  try {
